 ...
 *
 * Gleiches Tempo wie das Showreel (110,77 bpm), aber halb so lange Bilder:
 * vier Schläge sind eine Szene (65 Bilder). Die Anzeige läuft von der ersten
 * Sekunde an mit vollem Schlagwerk — bei einer Anzeige gibt es kein Vorspiel,
 * wer nach zwei Sekunden nicht hängen bleibt, ist weg.
 *
...
const SZENE = 65;
const ANZAHL_SZENEN = 5;
const ENDKARTE = 130;

const GESAMT_BILDER = ANZAHL_SZENEN * SZENE + ENDKARTE;
const SCHLAG = SZENE / 4 / FPS; // 0,5417 s — vier Schlaege je Szene
...
for (let s = 0; s < ANZAHL_SZENEN; s++) {
  const akkord = AKKORDE[s];
  const start = s * 4;

  einschlag(zeit(start), s === 0 ? 1.15 : 0.7);
  flaeche(zeit(start), 4 * SCHLAG, akkord.toene, 1);
  ...
  for (let b = 0; b < 4; b++) {
    ...
    zupfen(t, akkord.arp[b % 4], s === 0 ? 0.75 : 1, -0.25);
    if (s >= 2) zupfen(t + SCHLAG * 0.5, akkord.arp[(b + 2) % 4], 0.65, 0.3);
  }
}

// Endkarte: ein letzter Schlag, vier Schlaege Rhythmus, dann steht nur die
// Flaeche — Ruhe unter dem Aufruf zum Formular.
const AB = ANZAHL_SZENEN * 4;
einschlag(zeit(AB), 1.25);
flaeche(zeit(AB), 8 * SCHLAG, [HZ.C3, HZ.Eb3, HZ.G3, HZ.C4], 1.2);
